...
RULES = [
    ...
    (r"\bgh\s+repo\s+delete\b", "deny",
     "COS bash guard: repository deletion is denied."),
    (r"\bgh\s+api\b[^\n|;&]*(-X\s*DELETE|--method[= ]\s*DELETE)", "deny",
     "COS bash guard: destructive gh api verbs are denied."),
    (r"\bgh\s+auth\s+logout\b", "ask",
     "COS bash guard: logging out breaks the pinned identity; confirm."),
    (r"\bgh\s+pr\s+merge\b", "ask",
     "COS bash guard: manual merges are ask-gated; tools/cos_ship.py is the sanctioned path."),
    (r"\bgit\s+push\b", "ask",
